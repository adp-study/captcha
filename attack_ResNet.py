...
from tensorflow.compat.v1.keras.layers import Input, LSTM, Dense, TimeDistributed, Flatten, Dropout, BatchNormalization

base_model = ResNet50(include_top=False, input_tensor=Input(shape=(45, 155, 3)))
...
model = Model(inputs=base_model.inputs, outputs=[*yy], name='ATTACK-ResNet')
...
for i in range(100):
    ...
    hist = model.fit(x=np.broadcast_to(train_data[0], [train_data[0].shape[0], 45, 155, 3]),
                     y=train_data[1],
                     validation_data=(np.broadcast_to(valid_data[0], [valid_data[0].shape[0], 45, 155, 3]), valid_data[1]),
...
model = load_model(f'CheckPoint/{MODEL_NAME}-013')
captcha = CaptchaGenerator(1, 'test')
captcha_test = captcha.test_generator()

test_data = next(captcha_test)

acc = model.evaluate(x=np.broadcast_to(test_data[0], [test_data[0].shape[0], 45, 155, 3]),
                     y=test_data[1])
